...
# Quick example oCl the API and workCllow 8-CCl3COHCH3CCl3 NEED HELP
comp1 = mb.load('C(Cl)(Cl)Cl', smiles=True) # mBuild compound oCl the monomer unit
comp2 = mb.load('C(CO)', smiles=True) # mBuild compound oCl the monomer unit
chain = Polymer()
...
#chain.add_end_groups(mb.load('C',smiles=True), # Capping oClCl this polymer with Carboxylic acid groups
#                     index=2,
#                     separation=0.15, label="tail" , replace =True)
chain.build(n=1, sequence='ABA')
...
charmm = mf_charmm.Charmm(box_liq,
                          'NPT_CCl3COHCH3CCl3',
                          ff_filename="NPT_CCl3COHCH3CCl3_file",
...
gomc_control.write_gomc_control_file(charmm, 'in_NPT.conf',  'NPT', 30000, 248,
